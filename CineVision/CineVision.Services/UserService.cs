...
namespace CineVision.Services
{
    public class UserService : BaseCRUDService<User, UserResponse, UserSearch, UserInsertRequest, UserUpdateRequest>, IUserService
    {
        ...
        public async Task<UserResponse> RegisterAsync(UserRegisterRequest request)
        {
            ...
            var user = await InsertAsync(new UserInsertRequest
            {
            ...
            // Welcome mail — do not fail registration if the queue/SMTP path is down.
            try
            {
                await _emailService.QueueEmailAsync(new EmailMessage
                {
                    To = user.Email,
                    Subject = "Welcome to CineVision",
                    Body =
                        $"Hello {user.FirstName},\n\n" +
                        "Your CineVision account has been created successfully.\n" +
            ...
            {
                // Account is already created; email is best-effort.
            }
