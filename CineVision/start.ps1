...
if (-not $ready) {
    Write-Host "API is not healthy yet. Check logs with: docker compose logs -f cinevision-api" -ForegroundColor Red
    exit 1
}
...
Write-Host "  Desktop:  cd UI\cinevision_desktop ; flutter run -d windows"
Write-Host "  Mobile:   cd UI\cinevision_mobile  ; flutter run"
Write-Host ""
